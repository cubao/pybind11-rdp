...
@my_timer
def cpp_rdp(xyzs: np.ndarray, *, epsilon: float = 0.0, return_mask: bool = False) -> np.ndarray:
    return rdp_pybind(xyzs, epsilon=epsilon) if not return_mask else rdp_mask(xyzs, epsilon=epsilon)
